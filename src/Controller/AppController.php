    ...
    public function beforeRender(Event $event) {
        ...
        else
        {
            ...
            if(!empty($user_id))
            {
                ...
                $awaiting_payment = $this->TempCarts->find()->where(['TempCarts.user_id' => $user_id])->count();
                
                
                 $this->loadModel("Messages");                 
                $login_friends=$conn->execute("select U.id,U.business_name, U.full_name,U.is_active from users as U, messages as F where ( F.from_id = $user_id and F.to_id = U.id) or (F.to_id = $user_id and F.from_id = U.id) group by U.id")->fetchAll('assoc');
        ...
        $this->set(compact('user_details','interesrname','appCurController','SiteSettings', 'appCurActiion','appSlider','totReview','appReviews', 'appReviewsDt','revCount','appNews','provider_avg_review', 'user_id','awaiting_payment','login_friends'));
