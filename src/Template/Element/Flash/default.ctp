...
<div class="msg_box" style="right:290px;display:none;">
	<div class="msg_head text-white"><span id="chat_name"></span>
	...
	<div class="msg_wrap"> 
		<div class="msg_body p-0">
         ...
         <div class="msg_footer"><textarea class="msg_input" rows="1"></textarea><button onclick="send()"><i class="ion-ios-paperplane"></i></button></div>
</div>
</div>
...
<?php if($userid){ ?>
  ...
  <div class="chat_head up_chk" data-id="1"> <div class="col-md-10 col-sm-5">Chat Box</div><div align="right" id="chat_icon_up"><i class="" aria-hidden="true"></i></div></div>
    ...
    <?php foreach($login_friends as $logf){
    	...
    	?>
   <a href="javascript: select_friend_foot(<?php echo $logf['id'];?>)"> <div class="user" data-id=<?php echo $logf['id'];?>> <?php if($logf['is_active']==1){?> <span class="on_line on_<?php echo $logf['id'];?>" ><i class="fa fa-circle" aria-hidden="true"></i></span><?php }else{ ?><span class="off_line off_<?php echo $logf['id'];?>"><i class="fa fa-circle" aria-hidden="true"></i></span><?php } ?><label style="margin-left: 17px;"><?php echo $logf['full_name'];?></label></div></a>
    <?php } ?>
	</div>
  </div>
  <?php } ?>
