  ...
  //Menu pages start  
    ...
    public function index() { 
       ...
       // pr($services);
        ...
        $how_it_work = $this->Contents->find()->select(['Contents.content', 'Contents.page_title'])->where(['Contents.is_active' => 1, 'Contents.id' => 21])->first();

        $banner = $this->Sliders->find()->where(['Sliders.is_active' => 1])->first();

         $this->set(compact('services', 'testimonials', 'how_it_work', 'banner'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($flag) {
                ...
                if ($rs = $this->Users->save($user)) {
                    ...
                    $email = new Email('default');
                    $email->emailFormat('html')->from(['nit.spandan@gmail.com'=>'StarCity'])
                            ->to($mail_To)
                            ->subject($mail_subject)
                            ->send($mail_body);
...
//        }
        ...
        if ($userExist) {
          ...
          $this->Flash->error('Your account not exist.');
 ...
 //exit;
            ...
            if (!empty($userExist)) {
                ...
                $email = new Email('default');
                if ($email->emailFormat('html')->from(['nit.spandan@gmail.com' => 'StarCity'])
                                ->to($userExist['email'])
                                ->subject($mail_subject)
                                ->send($mail_body)) {
    ...
    public function dashboard($type=NULL) {
        ...
        if($uid!='' && isset($uid)){
          ...
          //$like= $this->Likes->find()->contain(['Posts'])->count('');
          ...
          if($type == 'U'){
           ...
           $services = $this->Posts->find()->contain(['Users', 'Comments', 'Likes', 'Reports','ServiceImages'])->where(['Posts.status' => 1, 'Posts.exp_date >=' => $date])->order(['Posts.update_date'=>'DESC'])->toArray();
          }
           //print_r($services);exit;
           
        $this->set(compact('title', 'user_info', 'services', 'uid','type'));
...
public function servicedashboard($type=NULL) {
        $this->viewBuilder()->layout('default');
        $title="Dashboard";
        $this->loadModel('Users');
        $this->loadModel('Services');
...
////add certficate
 ...
 if($flag){
    ...
    public function message(){
     ...
     $all_users = $conn->execute("select U.id, U.pimg, U.full_name,U.business_name, U.is_active, F.message, F.date_time,(SELECT count(*) FROM messages where messages.from_id=U.id AND messages.to_id = $user_id AND messages.is_read = 'N'  ) as read_count from users as U, messages as F where ( F.from_id = $user_id and F.to_id = U.id AND other_message != $user_id) or (F.to_id = $user_id and F.from_id = U.id AND other_message != $user_id) group by U.id order by F.id desc")->fetchAll('assoc');
     ...
     foreach ($all_users as $user) {
       //print_r($message);
    // print_r($user);
        $msg = $conn->execute("select message, image, date_time from messages where ( other_message != ".$user_id." AND  from_id =".$user['id']." AND to_id = ".$user_id.") or ( other_message !=".$user_id."  AND from_id=".$user_id." AND to_id=".$user['id']." ) order by id desc limit 0,1")->fetchAll('assoc');


// SELECT * FROM `messages` WHERE (from_id = 103 and to_id = 102 and other_message != 103) or ( to_id=103 and from_id=102 and other_message != 103) order by id desc limit 0,1
        ...
        $messages[] = array('id' => $user['id'], 'pimg' => $user['pimg'], 'full_name' => $user['full_name'],'business_name' => $user['business_name'], 'message' => $message, 'date_time' => $msg[0]['date_time'], 'read_count' => $user['read_count']);
     }
...
//print_r($messages);exit;
    ...
    public function footerusermessage()
      {
          ...
          $conn = ConnectionManager::get('default');  
           $conn->execute("UPDATE `messages` SET `is_read`= 'Y' WHERE `to_id` = '".$userid."' and `from_id` = '".$friend_id."'");
      		$all_messages = $conn->execute("select U.id as uid, U.full_name, U.pimg, M.id, M.date_time, M.message,M.image from users as U, messages as M where (M.from_id =".$userid." and M.to_id = ".$friend_id." and M.from_id=U.id and M.other_message != ".$userid.") or (M.to_id =".$userid." and M.from_id = ".$friend_id." and M.from_id=U.id and M.other_message != ".$userid.") order by M.id desc limit 0,20")->fetchAll('assoc');
      		$message = "";
 ...
 public function latestMessagefooter() {
        ...
        $lmessage_id = $this->request->data['last_message_id'];
    		...
    		$all_messages = $conn->execute("select U.id as uid, U.full_name, U.pimg, M.id, M.date_time, M.message,M.image from users as U, messages as M where (M.from_id =".$userid." and M.to_id = ".$friend_id." and M.other_message != ".$userid." and M.from_id=U.id) or (M.to_id =".$userid." and M.from_id = ".$friend_id." and M.other_message != ".$userid." and M.from_id=U.id) having M.id > ".$lmessage_id." order by M.id desc")->fetchAll('assoc');
    		}
		$message = "";
		if(count($all_messages) > 0){
			$all_messages = array_reverse($all_messages);
...
$this->request->data['data']['Post']['update_date'] = date('Y-m-d H:i:s');
$this->request->data['data']['Post']['description'] = base64_encode($this->request->data['data']['Post']['description']);
